...
function SignUpForm() {
  ...
  useEffect(() => {
    ...
    if (!storedIsAuthenticated || !storedIsAdmin) {
      console.error('Failed to retrieve data from sessionStorage');
    }
  }, []);
  ...
  const handleRegister = (e) => {
    e.preventDefault();
    sessionStorage.setItem('users', JSON.stringify(users)); // Assuming 'users' is defined elsewhere
  ...
  const handleLogin = (e) => {
    ...
    if (!signInFormData.username || !signInFormData.password) {
      ...
      setIsAdmin(signInFormData.username === 'admin');
